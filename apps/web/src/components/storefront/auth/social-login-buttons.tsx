...
import {
  ...
  useStorefrontAuth,
  type AuthProvider,
} from "@/lib/store/storefront-auth-store";
...
const providers: {
...
}[] = [
  {
    ...
    className: "border-border/80 bg-background hover:bg-muted/60",
    icon: (
  ...
  },
  {
    ...
    className: "border-[#1877F2]/30 bg-[#1877F2]/5 hover:bg-[#1877F2]/10",
    icon: (
  ...
  },
  {
    ...
    className: "border-[#25D366]/30 bg-[#25D366]/5 hover:bg-[#25D366]/10",
    icon: (
...
export function SocialLoginButtons({ onSuccess }: SocialLoginButtonsProps) {
  ...
  const handleSocial = async (provider: SocialProvider) => {
    ...
    setUser(mockSocialUser(provider));
    toast.success(`Signed in with ${provider}`, { id: provider });
    onSuccess?.();
  };
    ...
    <div className="space-y-2">
      {providers.map((p) => (
        <Button
          ...
          type="button"
          variant="outline"
          className={cn("h-10 w-full justify-start gap-3 text-sm font-medium", p.className)}
