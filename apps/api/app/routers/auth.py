...
def _user_read(user: AuthUser) -> UserRead:
    return UserRead(id=user.id, email=user.email, username=user.username, name=user.name, role=user.role, is_active=user.is_active)
...
def login(payload: LoginRequest, db: Session = Depends(get_db)) -> LoginResponse:
    ...
    user = (
        db.query(AuthUser).filter(AuthUser.email == identifier).first()
        or db.query(AuthUser).filter(AuthUser.username == identifier).first()
    )
